...
const Navbar: React.FC = () => {
    ...
    const role = useRoleContext();  // Get role context safely
    const serverLabel = useLabels();
    const { handleLogOut } = useLogOut();

    // If role is not ready, don't render Navbar yet
    if (!role) return null;
    ...
    return (
        <section className="nav-container card">
            <nav className="nav-links">
                ...
                <NavLink className="link" to="/create">{role.differentDisplay}</NavLink>
                {currentUser && (
                    <>
                        {currentUser.role === serverLabel.role.level3[0] && (
                            <NavLink className="link" to="/upload">{Label.navBar.upload}</NavLink>
